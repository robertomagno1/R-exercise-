...
for (b in 1:B){
  ...
  x.boot <- x1[idx]
  tau.nboot[b] <- 1/(1 + psi.ml(x.boot))
}

# Bias via Nonparametric Bootstrap (lower than before)
cat('--- NBoot / Bias ----\n')
round(mean(tau.nboot) - tau.ml(x1), 3)

# SE via Nonparametric Bootstrap (lower than before)
cat('--- NBoot / SE ------\n')
